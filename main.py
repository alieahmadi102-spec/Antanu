...
AUTO_SEARCH_WORDS = [
    ...
    # درخواستِ منبع/رفرنسِ تازه — تا مدل به‌جای تکیه به دانشِ منجمدش، جست‌وجوی
    # زنده کند و به منبعِ ساختگی نیاز نداشته باشد.
    "منبع جدید", "منابع جدید", "رفرنس جدید", "مقاله جدید", "مقالات جدید",
    "پژوهش جدید", "تحقیق جدید", "مطالعه جدید", "منابع ۲۰۲۵", "منابع ۲۰۲۶",
    "منبع 2025", "منبع 2026", "sources 2025", "sources 2026",
]
...
BASE_SYSTEM_PROMPT = (
    ...
    "۱۵) صداقتِ ارجاع — بسیار مهم: هرگز منبع، مقاله، نویسنده یا شناسه‌ی DOI ساختگی را به‌عنوان واقعی معرفی نکن، "
    "حتی اگر شبیهِ یک منبعِ واقعی به‌نظر برسد. اگر برای یافتنِ منابع، نتایجِ جست‌وجوی وب یا جست‌وجوی علمی "
    "(Crossref/OpenAlex) در پرامپت داده شده، ارجاع‌ها را فقط از همان‌ها بساز. اگر چنین نتایجی داده نشده و "
    "کاربر منبعِ تازه (مثلاً سال ۲۰۲۵ یا ۲۰۲۶) خواسته، به‌جای رد کردنِ کامل با «به آن سال دسترسی ندارم» یا "
    "ساختنِ منبعِ فرضی، صریح بگو که بدونِ جست‌وجوی زنده نمی‌توانی صحتِ منبعِ تازه را تضمین کنی و پیشنهاد بده "
    "کاربر جست‌وجوی وب یا ابزارِ «ارجاع‌دهی» را روشن کند تا منبعِ واقعی پیدا شود."
)
...
async def api_longdoc(request: Request):
    ...
    async def gen():
        ...
        try:
            ...
            # منابع علمیِ واقعی: به‌جای اینکه بخشِ «منابع» را مدل از دانشِ منجمدش
            # بسازد (که دقیقاً همان چیزی است که به منبعِ ساختگی می‌انجامد)، همین
            # الان با Crossref/OpenAlex جست‌وجوی واقعی می‌کنیم و مدل فقط همین‌ها را
            # فهرست می‌کند.
            yield log("⏳ جست‌وجوی منابع علمی واقعی…\n")
            real_refs = []
            try:
                real_refs = await lookup_reference_online(topic)
            except Exception:
                real_refs = []
            refs_digest = ""
            if real_refs:
                yield log(f"✅ {len(real_refs)} منبع علمیِ واقعی پیدا شد.\n")
                lines = []
                for it in real_refs:
                    who = it.get("authors") or "؟"
                    lines.append(f"- {who} ({it.get('year') or '؟'}). {it.get('title') or ''}. "
                                f"{it.get('source') or ''}. {it.get('url') or ''}")
                refs_digest = "\n".join(lines)
            else:
            ...
            for i, t in enumerate(titles, 1):
                ...
                if stats_results and t == _METHOD_SECTION_TITLE:
                    ...
                    if refs_digest:
                        data_note = (
                            "\n\nاین بخش را با این منابعِ واقعی (از Crossref/OpenAlex) به سبک APA فارسی "
                            f"فهرست کن — دقیقاً همین‌ها، نه بیشتر و نه ساختگی:\n{refs_digest}\n"
                            "اگر منابعِ فارسیِ معتبر دیگری هم از دانش خودت مطمئنی، می‌توانی اضافه کنی؛ "
                            "ولی هرگز DOI یا نویسنده‌ی ساختگی نساز."
                        )
                    else:
                        data_note = (
                            "\n\nجست‌وجوی خودکار منبع علمی برای این موضوع نتیجه‌ای نداد. اگر منبعِ واقعی و "
                            "مطمئنی از دانش خودت داری بیاور؛ در غیر این صورت صادقانه بنویس که منبعِ "
                            "تأییدشده‌ای در دسترس نبود — هرگز منبع یا DOI ساختگی نساز."
                        )
